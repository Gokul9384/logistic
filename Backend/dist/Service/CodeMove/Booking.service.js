...
const _bookings = require("../../Database/Table/CodeMove/bookings");
...
let BookingService = class BookingService {
    async GetAll() {
        return await _bookings.bookings.find({
            relations: [
                "order",
                "vendor",
                "quote",
    ...
    async GetById(Id) {
        const data = await _bookings.bookings.findOne({
            where: {
                id: Id
            },
            relations: [
        ...
        if (!data) throw new Error(_ResponseEnum.ResponseEnum.NotFound);
        return data;
    }
    ...
    `);
        ...
        const _BookingData = new _bookings.bookings();
        _BookingData.booking_number = BookingNumber[0].booking_number;
        _BookingData.order_id = BookingData.order_id;
        _BookingData.vendor_id = BookingData.vendor_id;
        _BookingData.customer_id = BookingData.customer_id;
        _BookingData.quote_id = BookingData.quote_id;
        _BookingData.amount = BookingData.amount;
        _BookingData.booking_status = _BookingStatusEnum.BookingStatusEnum.CREATED;
        _BookingData.driver_id = BookingData.driver_id;
        _BookingData.vehicle_number = BookingData.vehicle_number;
        _BookingData.created_by_id = UserId;
        _BookingData.created_on = new Date();
        await _bookings.bookings.insert(_BookingData);
        // Vendor Notification
        const newVendorNotification = new _notification.notification();
        newVendorNotification.title = 'New Booking Created';
        ...
        newVendorNotification.created_by_id = UserId;
        newVendorNotification.created_on = new Date();
        await _notification.notification.insert(newVendorNotification);
        // Customer Notification
        const newCustomerNotification = new _notification.notification();
        newCustomerNotification.title = 'New Booking Created';
        ...
        newCustomerNotification.created_by_id = UserId;
        newCustomerNotification.created_on = new Date();
        await _notification.notification.insert(newCustomerNotification);
        // Audit log
        this._AuditLogService.AuditEmitEvent({
            PerformedType: _bookings.bookings.name,
    ...
    async Update(Id, BookingData, UserId, UserIp) {
        const UpdateData = await _bookings.bookings.findOne({
            where: {
                id: Id
            }
        });
        if (!UpdateData) throw new Error(_ResponseEnum.ResponseEnum.NotFound);
        ...
        UpdateData.updated_by_id = UserId;
        UpdateData.updated_on = new Date();
        await _bookings.bookings.update(Id, UpdateData);
        // If status changed to Confirmed and driver is assigned
        if (oldStatus !== _BookingStatusEnum.BookingStatusEnum.CONFIRMED && BookingData.booking_status === _BookingStatusEnum.BookingStatusEnum.CONFIRMED && BookingData.driver_id) {
            // Update order status
            ...
            WHERE id = ?
        `, [
                _OrderStatusEnum.OrderStatusEnum.Assigned,
                UpdateData.order_id
            ]);
            // Create or update delivery entry
            const existingDelivery = await _delivery.delivery.findOne({
                where: {
            ...
            if (existingDelivery) {
                ...
                await _delivery.delivery.update(existingDelivery.id, {
                    driver_id: BookingData.driver_id,
                    delivery_status: _DeliveryStatusEnum.DeliveryStatusEnum.Assigned,
                    updated_by_id: UserId,
                    updated_on: new Date()
                });
            } else {
                // Create new delivery entry
                await _delivery.delivery.insert({
            ...
            // Notify driver
            const newDriverNotification = new _notification.notification();
            ...
            await _notification.notification.insert(newDriverNotification);
        }
        ...
        if (oldDriverId !== BookingData.driver_id && BookingData.driver_id) {
            ...
            await this._DataSource.getRepository(_delivery.delivery).update({
                booking_id: UpdateData.id
            }, {
                driver_id: BookingData.driver_id,
                updated_by_id: UserId,
