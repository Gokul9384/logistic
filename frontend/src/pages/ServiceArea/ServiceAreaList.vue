<template>
  <q-page padding>
    <div class="page-header row items-center justify-between q-mb-lg">
      <h2 class="text-h5 text-weight-medium q-ma-none">Service Area Management</h2>
    ...
    <q-card flat bordered>
      <q-table
        ...
        :rows="paginatedServiceAreas"
        row-key="id"
        v-model:pagination="tablePagination"
        :rows-per-page-options="pageSizeOptions"
        @request="onTableRequest"
        :loading="isLoading"
        class="service-areas-table"
      >
        ...
        <template v-slot:body-cell-actions="props">
          <q-td :props="props" class="text-center">
            <div class="action-buttons q-gutter-xs">
              ...
              >
                <q-tooltip>Edit Service Area</q-tooltip>
              </q-btn>
              <q-btn
                flat
                dense
              ...
              >
                <q-tooltip>Delete Service Area</q-tooltip>
              </q-btn>
            </div>
          </q-td>
        </template>
...
const filteredServiceAreas = computed(() => {
  let filtered = allServiceAreas.value;

  if (searchQuery.value.trim()) {
    const searchTerm = searchQuery.value.toLowerCase().trim();
    filtered = allServiceAreas.value.filter((serviceArea: any) => {
...
const paginatedServiceAreas = computed(() => {
  const filtered = filteredServiceAreas.value;
...
const fetchServiceAreas = async () => {
  try {
    ...
    const response = await CommonService.GetAll('/v1/ServiceArea/List');
    allServiceAreas.value = response;
...
const navigateToAdd = () => {
  router.push('/service_area/0');
};

const navigateToEdit = (serviceAreaId: string) => {
  router.push(`/service_area/${serviceAreaId}`);
};

const confirmDelete = (serviceArea: any) => {
  $q.dialog({
    title: 'Are you sure?',
    message: `You want to ${serviceArea.status ? 'deactivate' : 'activate'} service area "${serviceArea.name}"!`,
    persistent: true,
    color: 'warning',
    ok: {
      label: 'Confirm',
    ...
    try {
      ...
      const response = await CommonService.CommonDelete(`/v1/ServiceArea/Delete/${serviceArea.id}`);
      ...
      CommonHelper.ErrorToaster('Failed to update service area status');
    } finally {
